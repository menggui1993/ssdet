...
#     if iou_type == 'giou' or iou_type == 'diou' or iou_type == 'ciou':
#         cw = torch.max(bboxes1[:, None, 2], bboxes2[:, 2]) - torch.min(bboxes1[:, None, 0], bboxes2[:, 0])
#         ch = torch.max(bboxes1[:, None, 3], bboxes2[:, 3]) - torch.min(bboxes1[:, None, 1], bboxes2[:, 1])
#         if iou_type == 'giou':
#             c_area = cw * ch
#             giou = iou - (c_area - union) / c_area
#             return giou
#         if iou_type == 'diou' or iou_type == 'ciou':
#             c2 = cw ** 2 + ch ** 2
#             d2 = ((bboxes2[:, 0] + bboxes2[:, 2]) - (bboxes1[:, None, 0] + bboxes1[:, None, 2])) ** 2 / 4 + ((bboxes2[:, 1] + bboxes2[:, 3]) - (bboxes1[:, None, 1] + bboxes1[:, None, 3])) ** 2 / 4
#             if iou_type == 'diou':
#                 diou = iou - d2 / c2
#                 return diou
#             if iou_type == 'ciou':
